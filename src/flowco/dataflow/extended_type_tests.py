...
#     TypedDictType, PDDataFrameType, PDSeriesType, NumpyNdarrayType,
...
# -----------------------
# Test Cases for TypedDictType
# -----------------------


def test_typed_dict_type_valid():
    ...
    try:
        typed_dict_type.check_value({"name": "Alice", "age": 30})
    except ValueError:
        pytest.fail(
            "TypedDictType.check_value() raised ValueError unexpectedly for valid TypedDict."
        )


def test_typed_dict_type_missing_key():
    typed_dict_type = TypedDictType(
        name="Person",
        items=[
            KeyType(key="name", type=StrType(), description="Person's name"),
    ...
    with pytest.raises(ValueError) as exc_info:
        typed_dict_type.check_value({"name": "Alice"})
    assert "Missing key 'age' in dictionary" in str(exc_info.value)


def test_typed_dict_type_invalid_value():
    typed_dict_type = TypedDictType(
        name="Person",
        items=[
            KeyType(key="name", type=StrType(), description="Person's name"),
    ...
    with pytest.raises(ValueError) as exc_info:
        typed_dict_type.check_value({"name": "Alice", "age": "thirty"})
    assert "Key 'age': Expected int, got str" in str(exc_info.value)


def test_typed_dict_type_extra_keys():
    ...
    try:
        typed_dict_type.check_value({"name": "Alice", "age": 30, "gender": "female"})
    except ValueError:
        pytest.fail(
            "TypedDictType.check_value() raised ValueError unexpectedly for dict with extra keys."
        )
...
def test_extended_type_from_value_valid():
    ...
    assert (
        ...
        == "TypedDict('AutoGeneratedTypedDict', {'name': str, 'age': int})"
    )
    try:
...
def test_extended_type_from_value_nested():
    ...
    extended_type = ExtendedType.from_value(value)
    assert "TypedDict" in extended_type.to_python_type()
    try:
        extended_type.check_value(value)
    except ValueError:
        pytest.fail(
...
def test_extended_type_any_type():
    ...
    assert (
        extended_type.to_python_type()
        == "TypedDict('AutoGeneratedTypedDict', {'anything': List[Any], 'another': TypedDict('AutoGeneratedTypedDict', {'nested': bool})})"
    )
    try:
        extended_type.check_value(value)
    except ValueError:
...
def test_empty_typed_dict_type():
    typed_dict_type = TypedDictType(name="EmptyDict", items=[])
    try:
        typed_dict_type.check_value({})
    except ValueError:
        pytest.fail(
            "TypedDictType.check_value() raised ValueError unexpectedly for empty TypedDict."
        )
...
def test_extended_type_nested_list_of_dicts():
    ...
    assert (
        extended_type.to_python_type()
        == "List[TypedDict('AutoGeneratedTypedDict', {'id': int, 'name': str})]"
    )
    try:
        extended_type.check_value(value)
    except ValueError:
...
def test_extended_type_mixed_types():
    ...
    extended_type = ExtendedType.from_value(value)
    assert extended_type.to_python_type().startswith("TypedDict")
    try:
        extended_type.check_value(value)
    except ValueError:
        pytest.fail(
...
# -----------------------
# Test Cases for ExtendedType with TypedDict and DictType
# -----------------------


def test_extended_type_typedict_vs_dict():
    # TypedDict with string keys
    typed_dict_value = {"name": "Alice", "age": 30}
    extended_typed_dict = ExtendedType.from_value(typed_dict_value)
    assert isinstance(extended_typed_dict.the_type, TypedDictType)
    ...
    try:
        extended_typed_dict.check_value(typed_dict_value)
    except ValueError:
        pytest.fail(
            "ExtendedType.check_value() raised ValueError unexpectedly for valid TypedDict."
        )
    ...
    # Invalid TypedDict
    invalid_typed_dict = {"name": "Alice", "age": "thirty"}  # age should be int
    with pytest.raises(ValueError) as exc_info:
        extended_typed_dict.check_value(invalid_typed_dict)
    assert "Key 'age': Expected int, got str" in str(exc_info.value)
